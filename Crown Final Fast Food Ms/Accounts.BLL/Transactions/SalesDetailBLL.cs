...
namespace Accounts.BLL
{
    public class SalesDetailBLL
    {
        ...
        public List<SaleDetailEL> GetPartyWiseSaleSummaryForDistribution(Int64 IdProject, Int64 BookNo, string OrderBooker, string DeliveryMan, DateTime startDate, DateTime endDate)
        {
            SqlConnection objconn = new SqlConnection(DBHelper.DataConnection);
            try
            {
                objconn.Open();
                return dal.GetPartyWiseSaleSummaryForDistribution(IdProject, BookNo, OrderBooker, DeliveryMan, startDate, endDate, objconn);
            }
            catch (Exception ex)
            {
                objconn.Close();
