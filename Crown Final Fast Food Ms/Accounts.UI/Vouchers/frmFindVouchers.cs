...
namespace Accounts.UI
{
    public partial class frmFindVouchers : MetroForm
    {
        ...
        private void CreateVoucherWiseColumns()
        {
            ...
            DataGridViewButtonColumn colPrint = new DataGridViewButtonColumn();
            colStatus.HeaderText = "...";
            colStatus.Name = "colPrint";
            colStatus.Width = 75;
            this.DgvVoucher.Columns.Add(colPrint);

            if (VoucherType == "SaleInvoiceVoucher")
            {
                colPrint.Visible = true;
            }
            else
            {
                colPrint.Visible = false;
            }
        }
        #endregion
        ...
        private void DgvVoucher_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.ColumnIndex == 11)
            {
                Int64 Id = Validation.GetSafeLong(DgvVoucher.Rows[e.RowIndex].Cells[0].Value);
                PrintReport(Id);
            }
        }
