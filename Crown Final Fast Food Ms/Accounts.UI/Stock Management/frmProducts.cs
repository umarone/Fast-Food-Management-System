...
namespace Accounts.UI
{
    public partial class frmProducts : MetroForm
    {
        ...
        public bool IsFromOtherSide { get; set; }
        #endregion
        ...
        private bool ValidateControls()
        {
            ...
            else if (cbxItemType.Text == "Finished Good")
            {
                if (txtMRP.Text == string.Empty || Validation.GetSafeDecimal(txtMRP.Text) == 0)
                {
                    MessageBox.Show("Please Enter Sale Price for Finished Good");
                    isValid = false;
                }
            }
            return isValid;
        ...
        private bool ValidateBarCode()
        {
            ...
            //{
            //    var manager = new ItemsBLL();
            //    List<ItemsEL> list = manager.GetItemByBarCode(Operations.IdProject, txtBarCode.Text);
            //    if (list.Count > 0)
            //        return false;
            //    else
            //        return true;
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            if (ValidateControls())
            {
                if (ValidateBarCode())
                {
                    ...
                    if (cbxItemType.Text == "Raw Item")
                    {
                        oelItem.ItemType = 1;
                    }
                    else
                        oelItem.ItemType = 2;
                    if (chkDealItem.Checked)
                        oelItem.IsDeal = true;
                    else
                        oelItem.IsDeal = false;
                    //oelItem.PackingSize = "";
        ...
        private void GetItem(Int64? Id)
        {
            ...
            if (list.Count > 0)
            {
                ...
                cbxItemType.SelectedIndex = list[0].ItemType;
                chkDealItem.Checked = list[0].IsDeal.Value;
