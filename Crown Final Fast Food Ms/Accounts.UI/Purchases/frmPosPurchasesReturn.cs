...
namespace Accounts.UI
{
    public partial class frmPosPurchasesReturn : Form
    {
        ...
        private void frmPosPurchasesReturn_KeyPress(object sender, KeyPressEventArgs e)
        {
        ...
        private void FillData()
        {
            ...
            txtInvoiceNumber.Text = Validation.GetSafeString(manager.GetMaxPurchaseReturnNumber(Operations.IdProject, Operations.BookNo));
        }
        private void LoadAllItems()
        {
            var manager = new ItemsBLL();
            List<ItemsEL> list = manager.GetAllItemsByType(Operations.IdProject, 1);
            if (list.Count > 0)
            {
        ...
        private bool ValidateControls()
        {
            ...
            if (cbxPurchasesType.SelectedIndex == 1)
            {
            ...
            if (CheckSoftwareTaxableNature())
            {
                ...
                {
                    MessageBox.Show("Tax Account Missing...");
                    return false;
                }
            }           
            return true;
            ...
            if (ValidateRows())
            {
                if (ValidateControls())
                {
                    if (ValidateBookPeriod())
                    {
                        ...
                        oelVoucher.TotalFreight = Validation.GetSafeDecimal(txtFreight.Text);
        ...
        private void txtInvoiceNumber_KeyPress(object sender, KeyPressEventArgs e)
        {
            ...
            {
                e.Handled = true;
                FillPurchasesReturnByNumber(Validation.GetSafeLong(txtInvoiceNumber.Text));
            }
        }
        ...
        private void FillProductInGrid(List<ItemsEL> list, Int64? Id)
        {
            ...
            if (list.Count > 0)
            {
                ...
                //txtInvoiceTotalAfterDiscount.Text = Amount.ToString("0.00");
                txtInvoiceTotalAfterDiscount.Text = Validation.GetSafeString(Validation.GetSafeDecimal(Amount) + (Validation.GetSafeDecimal(txtFreight.Text)));
                CalculateTax();
            }
        }
        #endregion
        ...
        private void FillVoucher()
        {
            ...
            List<VoucherDetailEL> ListPurchases = PManager.GetPOSPurchasesReturnTransactions(IdVoucher.Value, Operations.IdProject, Operations.BookNo);
            if (ListPurchases.Count > 0)
            {
                ...
                txtTotalItems.Text = Validation.GetSafeString(CommonFunctions.RemoveTrailingZeros(ListPurchases[0].TotalItems));
                txtInvoiceTotal.Text = Validation.GetSafeString(CommonFunctions.RemoveTrailingZeros(ListPurchases[0].BillAmount));
                txtFreight.Text = Validation.GetSafeString(CommonFunctions.RemoveTrailingZeros(ListPurchases[0].TotalFreight));
                txtDiscount.Text = Validation.GetSafeString(CommonFunctions.RemoveTrailingZeros(ListPurchases[0].TotalDiscount));
                txtInvoiceTotalAfterDiscount.Text = Validation.GetSafeString(CommonFunctions.RemoveTrailingZeros(ListPurchases[0].TotalAmount));
                ...
                {
                    cbxPurchasesType.SelectedIndex = 1;
                }
        ...
        private void FillPurchasesReturnByNumber(Int64 PNo)
        {
            ...
            //List<VouchersEL> list = Manager.GetVouchersByTypeAndVoucherNumber(Operations.IdCompany, VoucherType, Convert.ToInt64(VEditBox.Text));
            List<VoucherDetailEL> ListPurchases = PManager.GetPOSPurchasesReturnTransactionsByNumber(PNo, Operations.IdProject, Operations.BookNo);
            if (ListPurchases.Count > 0)
            {
                IdVoucher = ListPurchases[0].IdVoucher;
                txtInvoiceNumber.Enabled = false;
                ...
                List<VoucherDetailEL> listTransactions = PManager.GetPurchasesReturnSubTransactions(IdVoucher.Value, Operations.IdProject, Operations.BookNo, ListPurchases[0].IsNetTransaction.Value);
