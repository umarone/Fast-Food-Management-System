...
namespace Accounts.UI
{
    public partial class frmPosPurchases : Form
    {
        ...
        private void frmPosPurchases_KeyPress(object sender, KeyPressEventArgs e)
        {
        ...
        private void FillData()
        {
            ...
            txtInvoiceNumber.Text = Validation.GetSafeString(manager.GetMaxPurchaseNumber(Operations.IdProject, Operations.BookNo));
        }
        ...
        private void grdProducts_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.ColumnIndex == 6) // Rate
            {
                grdProducts.Rows[e.RowIndex].Cells["colAmount"].Value = Validation.GetSafeDecimal(grdProducts.Rows[e.RowIndex].Cells["colQuantity"].Value)
                                                                            * Validation.GetSafeDecimal(grdProducts.Rows[e.RowIndex].Cells["colUnitPrice"].Value);
            ...
            txtInvoiceTotalAfterDiscount.Text = Validation.GetSafeString(Validation.GetSafeDecimal(Amount) - (Validation.GetSafeDecimal(txtDiscount.Text)));
            CalculateTax();
        }
        ...
        private void txtInvoiceNumber_KeyPress(object sender, KeyPressEventArgs e)
        {
            ...
            {
                e.Handled = true;
                FillPurchasesByNumber(Validation.GetSafeLong(txtInvoiceNumber.Text));
            }
        }
        ...
        private void FillVoucher()
        {
            ...
            if (ListPurchases.Count > 0)
            {
                ...
                txtDiscount.Text = Validation.GetSafeString(CommonFunctions.RemoveTrailingZeros(ListPurchases[0].Discount));
        ...
        private void FillPurchasesByNumber(Int64 PNo)
        {
            ...
            if (ListPurchases.Count > 0)
            {
                ...
                txtTotalItems.Text = Validation.GetSafeString(CommonFunctions.RemoveTrailingZeros(ListPurchases[0].TotalItems));
                txtInvoiceTotal.Text = Validation.GetSafeString(CommonFunctions.RemoveTrailingZeros(ListPurchases[0].BillAmount));
                txtDiscount.Text = Validation.GetSafeString(CommonFunctions.RemoveTrailingZeros(ListPurchases[0].ExtraDiscount));
                txtFreight.Text = Validation.GetSafeString(CommonFunctions.RemoveTrailingZeros(ListPurchases[0].TotalFreight));
                txtInvoiceTotalAfterDiscount.Text = Validation.GetSafeString(CommonFunctions.RemoveTrailingZeros(ListPurchases[0].TotalAmount));
                //if (!ListPurchases[0].IsNetTransaction.Value)
                // {
        ...
        private void FillPurchases(List<VoucherDetailEL> List)
        {
            ...
            if (List != null && List.Count > 0)
            {
                ...
                for (int i = 0; i < List.Count; i++)
                {
                    ...
                    grdProducts.Rows[i].Cells[8].Value = List[i].Discount;
